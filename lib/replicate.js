...
Peer.prototype.onextension = function (id, message) {
  const name = this.remoteExtensions && this.remoteExtensions[id]
  if (name) this.feed.emit('extension', name, message, this)
}
...
Peer.prototype.onopen = function () {
  ...
  if (this.ack || (this.extensions && this.extensions.length)) {
    this.stream.options({
      ack: this.ack,
...
Peer.prototype._sendWants = function () {
  if (!this.wants || !this.downloading || !this._remoteOpened) return
...
Peer.prototype.extension = function (name, message) {
  const id = this.extensions ? this.extensions.indexOf(name) : -1
